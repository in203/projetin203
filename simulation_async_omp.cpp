...
// j'ai utuilisé ici la parallelisation car il s'agit d'un cpu bound ici (le nombre d'operation est plus grand que le nombre d'acces a la mémoire)
        #pragma omp parallel for schedule(static)//pour un nombre constant pour chaque thread
//#pragma omp parallel for schedule(dynamic) // pour un nombre global constatnt d'individus
