...
//stat = grille.getStatistiques();
    ...
    MPI_Recv(&jours_écoulés , 1, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD ,& status );
MPI_Recv(stat.data(), 3*largeur_grille*hauteur_grille, MPI_INT, MPI_ANY_SOURCE, 0, MPI_COMM_WORLD ,& status );
...
std::cout<<elapsed_seconds.count()<<std::endl;
        ...
        else{//proc respo de la simutlation
        ...
        //on va envoyer la grille et le jour a proc
        auto& stat = grille.getStatistiques();


        MPI_Send(&jours_écoulés, 1, MPI_INT, 0, 0, MPI_COMM_WORLD);
        MPI_Send(stat.data(), 3*largeur_grille*hauteur_grille, MPI_INT, 0, 0, MPI_COMM_WORLD);
